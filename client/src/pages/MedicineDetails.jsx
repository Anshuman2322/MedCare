      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-8">
            ...
            <div ref={contentRef} className={`flex-1 space-y-5 ${animationClasses.fadeRight(contentVisible)}`}>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {medicine?.requiresPrescription ? (
                  <div className="inline-flex items-center gap-1 text-xs font-bold text-red-700 bg-red-50 border border-red-200 rounded-full px-2.5 py-1">
                    <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M9 3h6l-1 9h4l-8 9 1-9H7z"/></svg>
                    Prescription Required
                  </div>
                ) : (
                  <div className="inline-flex items-center gap-1 text-xs font-semibold text-sky-700 bg-sky-50 border border-sky-200 rounded-full px-2.5 py-1">
              ...
              <div className="flex items-center gap-3 pt-4 flex-wrap">
                ...
                <button
                  type="button"
                  onClick={openInquiry}
                  className="inline-flex items-center justify-center gap-3 px-6 py-3 rounded-xl bg-emerald-700 text-white text-base font-medium shadow-sm hover:bg-emerald-800 transition-colors"
                >
                  <svg
        ...
        <div ref={tabsRef} className={`mt-2 ${animationClasses.fadeUp(tabsVisible)}`}>
          <div className="inline-flex rounded-lg bg-gray-100 p-1 gap-1">
            {[
              { key: 'Dosage', icon: <path d="M9 2h6M12 2v6M6 8h12l-1 12a2 2 0 01-2 2H9a2 2 0 01-2-2L6 8z" /> },
              { key: 'Usage', icon: <path d="M12 2a7 7 0 017 7c0 5-7 13-7 13S5 14 5 9a7 7 0 017-7zM12 12a3 3 0 100-6 3 3 0 000 6z" /> },
              { key: 'Details', icon: <path d="M12 8h.01M11 12h1v4h1M12 21a9 9 0 100-18 9 9 0 000 18z" /> },
            ].map(({ key, icon }) => (
              <button
                key={key}
                onClick={() => setTab(key)}
                className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-md text-sm font-medium transition ${tab === key ? 'bg-white shadow text-emerald-600' : 'text-gray-600 hover:text-gray-900'}`}
              >
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">{icon}</svg>
                {key}
              </button>
            ))}
          </div>
                ...
                return rows.length > 0 ? (
                  <div className="space-y-5">
                    {sections.map((section, sectionIndex) => {
                      ...
                      return (
                        <div key={`${section.title}-${sectionIndex}`} className="rounded-xl bg-gray-50 p-5 space-y-1 shadow-sm">
                          ...
                          <div className="divide-y divide-gray-200">
                            {items.map((row, idx) => (
                              <div
                                key={`${row.label}-${idx}`}
                                className="grid grid-cols-1 sm:grid-cols-[180px_1fr] md:grid-cols-[220px_1fr] gap-2 py-2"
                              >
